              ...
              <Fragment key={comparison.id}>
                <TableRow className="border-border/40">
                  ...
                  <TableCell className="max-w-[250px]">
                    {comparison.flag === "MULTIPLE_MATCHES" && comparison.possibleMatches ? (
                      <div className="text-xs">
                        <span className="text-blue-500 font-medium">Multiple matches found:</span>
                        <ul className="mt-1 space-y-0.5">
                          {comparison.possibleMatches.slice(0, 3).map((match, idx) => (
                            <li key={idx} className="text-muted-foreground truncate">
                              • {match.procedureName} ({Math.round(match.similarity * 100)}%)
                            </li>
                          ))}
                        </ul>
                      </div>
